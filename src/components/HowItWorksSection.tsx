    ...
    <section id="how-it-works" className="w-full max-w-6xl mx-auto px-4 py-20">
      <div className="text-center mb-16">
        <div className="inline-flex items-center bg-soft-peach/60 backdrop-blur-md border border-cream/40 rounded-full px-6 py-3 mb-8">
          <Sparkles className="w-5 h-5 text-terracotta mr-2" />
        ...
        <h2 className="text-5xl md:text-6xl font-poppins font-bold text-warm-brown mb-6">
          How Journely
          <span className="block bg-gradient-to-r from-terracotta to-peach bg-clip-text text-transparent">transforms your words</span>
        </h2>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        {steps.map((step, index) => (
          <Card key={index} className="bg-card-gradient backdrop-blur-md border-cream/30 p-6 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 relative group">
            <div className="absolute -top-4 -right-4 w-8 h-8 bg-gradient-to-br from-terracotta to-peach rounded-full flex items-center justify-center text-white font-bold text-sm">
              {step.step}
            </div>
            
            <div className="bg-soft-peach/60 w-14 h-14 rounded-2xl flex items-center justify-center mb-6">
